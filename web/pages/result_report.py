...
def render() -> None:
    ...
    # 브랜드 맵 수동 그리기: 매칭 테이블 편집 후 그래프 (PDF 분석 재실행 불필요)
    st.markdown("---")
    st.markdown("## 브랜드 맵 수동 그리기")
    st.caption("아래에서 **매칭 테이블**을 불러온 뒤 스타일_축·프리미엄_축·대표색을 수정할 수 있습니다. 가격·상품 수는 기존 분석 데이터 기준으로 집계됩니다. PDF 분석을 다시 돌리지 않고 맵만 조정할 수 있습니다.")
    if "brand_map_manual_table" not in st.session_state and BRAND_MAP_TABLE_PATH.exists():
        try:
            st.session_state["brand_map_manual_table"] = json.loads(BRAND_MAP_TABLE_PATH.read_text(encoding="utf-8"))
            st.session_state["brand_map_show_manual"] = True
        except Exception:
            pass
    if st.button("매칭 테이블 불러오기 (자동 집계)", key="load_brand_map_table"):
        from analysis.brand_map_data import build_brand_map_data
        tbl = build_brand_map_data(data)
        # 편집용 리스트: 브랜드, 스타일_축, 프리미엄_축, 대표색, 상품수, 평균가격
        rows = [
            {
                "브랜드": b["brand_name"],
                "스타일_축": int(b["style_axis"]),
                "프리미엄_축": int(b["premium_axis"]),
                "대표색": b["representative_color_hex"],
                "상품수": b["item_count"],
                "평균가격": b.get("avg_price") or "",
            }
            for b in tbl
        ]
        st.session_state["brand_map_manual_table"] = rows
        st.success("테이블을 불러왔습니다. 셀을 편집한 뒤 '테이블 저장' 또는 '이 테이블로 맵 그리기'를 누르세요.")
        st.rerun()
    manual_table = st.session_state.get("brand_map_manual_table")
    if manual_table:
        import pandas as pd
        ...
        edited = st.data_editor(
            ...
            column_config={
                ...
                "스타일_축": st.column_config.NumberColumn("스타일_축 (0~100)", min_value=0, max_value=100, step=1),
                "프리미엄_축": st.column_config.NumberColumn("프리미엄_축 (0~100)", min_value=0, max_value=100, step=1),
                "대표색": st.column_config.TextColumn("대표색 (hex 또는 색상명)"),
                "상품수": st.column_config.NumberColumn("상품수", disabled=True),
                "평균가격": st.column_config.NumberColumn("평균가격", disabled=True),
            },
            use_container_width=True,
            key="brand_map_manual_editor",
            num_rows="fixed",
        )
        st.session_state["brand_map_manual_table"] = edited.to_dict("records")
        col_save, col_draw, _ = st.columns([1, 1, 2])
        with col_save:
            if st.button("테이블 저장", key="save_brand_map_table"):
                OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
                BRAND_MAP_TABLE_PATH.write_text(
                    json.dumps(st.session_state["brand_map_manual_table"], ensure_ascii=False, indent=2),
                    encoding="utf-8",
                )
                st.success("저장되었습니다.")
        with col_draw:
            if st.button("이 테이블로 맵 그리기", key="draw_manual_map"):
                st.session_state["brand_map_show_manual"] = True
                st.rerun()
        if st.session_state.get("brand_map_show_manual"):
            from analysis.brand_map_data import _color_to_hex
            rows = st.session_state.get("brand_map_manual_table", [])
            if rows:
                names = [r.get("브랜드", "") for r in rows]
                style_axis = [int(r.get("스타일_축", 50)) for r in rows]
                premium_axis = [int(r.get("프리미엄_축", 50)) for r in rows]
                colors_hex = [_color_to_hex(str(r.get("대표색", "#757575"))) for r in rows]
                item_count = [int(r.get("상품수", 1)) for r in rows]
                max_count = max(item_count) if item_count else 1
                sizes = [12 + (c / max_count) * 28 for c in item_count]
                import plotly.graph_objects as go
                fig = go.Figure()
                fig.add_trace(go.Scatter(
                    ...
                    textposition="top center",
                    mode="markers+text",
                    marker=dict(size=sizes, color=colors_hex, opacity=0.85, line=dict(width=1, color="rgba(0,0,0,0.3)")),
                    textfont=dict(size=11),
                    name="",
                ))
                fig.update_layout(
